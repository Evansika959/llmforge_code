...
"""
    ...
    # Fixed model-architecture defaults pinned onto every remote training launch.
    # These are not in the NSGA search space yamls, so without pinning here the
    # remote train.py would fall back to its own argparse defaults, which differ
    # from the intended NSGA regime (SwiGLU MLP + RoPE + peri-LN, no abs-pos).
    _FIXED_MODEL_OVERRIDES: Dict[str, Any] = {
        "mlp_variant": "swiglu",
    ...
    def submit_job(self, path_to_yaml: str, remote_work_dir: str, dir_name: str, conda_env: str = "llmforge", max_iters: int = 10000, dataset: str = "minipile") -> bool:
        ...
        for i, host in enumerate(self.hosts):
            ...
            try:
                ...
                fixed_overrides_str = " ".join(
                    f"{k}={v}" for k, v in self._FIXED_MODEL_OVERRIDES.items()
                )
                override_args_str = f"max_iters={max_iters} {fixed_overrides_str}"
                ...
                cmd = (
                    ...
                    f"  conda run -n {conda_env} python -u optimization_and_search/run_from_yaml.py --yaml {remote_yaml_path} --output_dir {remote_run_dir} --prefix {base} --override_args {override_args_str} --dataset={dataset}; ec=$?;\n"
                    f"else\n"
                    ...
                    f"  python -V || true;\n"
                    f"  python -u optimization_and_search/run_from_yaml.py --yaml {remote_yaml_path} --output_dir {remote_run_dir} --prefix {base} --override_args {override_args_str} --dataset={dataset}; ec=$?;\n"
                    f"fi;\n"
