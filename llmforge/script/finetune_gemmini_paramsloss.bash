#!/bin/bash
# Production NSGA-II co-search — Gemmini HW backend variant.
...
#                                  in hw_eval/arch/system_gemmini.yaml.
#                                  Two-pass evaluation (prefill at
#                                  prefill_len, decode at decode_len) →
#                                  per-token totals.  Useful as a published-
#                                  silicon baseline alongside the in-house
#                                  DXE substrate.)
#
...
# decode_cycles assuming a 1 GHz clock (consistent with the dxe variant —
# raw cycle counts are also in aux for re-clocking).  energy_per_token_uJ
# averages prefill + decode energy across (prefill_len + decode_len)
# tokens.  Full per-arch breakdown — energy_uJ, cycles, total_ops,
# total_memory_accesses, fusion_saved_*, edp, prefill_*, decode_* — is
# preserved in aux for downstream analysis.
#
# Notes:
...
# - Gemmini's per-GEMM mapping cost is similar to DXE; Timeloop caches
#   GEMM-shape mappings, so cold-cache cost dominates the first ~20 unique
#   shapes.  Total Timeloop overhead similar to the DXE variant.
#
...
EXP_NAME="$(date +%Y%m%d)_finetune_200m_paramsloss_gemmini"
