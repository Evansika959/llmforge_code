#!/bin/bash
# Production NSGA-II co-search — FLAT (flat_edge) HW backend variant.
...
#                                 (Timeloop run against the FLAT edge-LLM
#                                  accelerator config — flat_edge in
#                                  hw_eval/arch/.  Two-pass evaluation
#                                  (prefill at prefill_len, decode at
#                                  decode_len) → per-token totals.  Useful
#                                  as an edge-LLM-specific baseline
#                                  alongside Eyeriss / Gemmini / DXE.)
#
...
# decode_cycles assuming a 1 GHz clock (same convention as the dxe,
# gemmini, and eyeriss variants — raw cycle counts are also in aux for
# re-clocking).  energy_per_token_uJ averages prefill + decode energy
# across (prefill_len + decode_len) tokens.  Full per-arch breakdown —
# energy_uJ, cycles, total_ops, total_memory_accesses, fusion_saved_*,
# edp, prefill_*, decode_* — is preserved in aux for downstream analysis.
#
# Notes:
...
# - FLAT is intended for edge-LLM-shaped GEMMs, so the mapper should
#   accept all shapes from our search space cleanly (similar to Eyeriss
#   / Gemmini, unlike the rigid `dxe` mapper).
#
...
EXP_NAME="$(date +%Y%m%d)_finetune_200m_paramsloss_flat"
