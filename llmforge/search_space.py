...
def _mlp_matrix_count(variant: str = "swiglu") -> int:
    """Weight matrices in the MLP block.

    remote_trainer.py pins mlp_variant='swiglu' for every trained model, so
    params / flops / mem_access estimators must all use 3 matrices
    (gate + up + down). Using 2 undercounts MLP by 33% of MLP cost.
    """
...
class Individual(dict):
    ...
    def estimate_params(self) -> int:
        ...
        for i in indices:
            ...
            mlp_params = _mlp_matrix_count(mlp_variant) * d * m
    ...
    def estimate_flops(self, seq_len: int = 512) -> int:
        ...
        for i in indices:
            ...
            if attn_variant == "infinite":
                ...
                raise ValueError(f"Unknown attention_variant: {attn_variant}")
            mlp_variant = li.get("mlp_variant", g.get("mlp_variant", "swiglu"))
            mlp = 2.0 * _mlp_matrix_count(mlp_variant) * seq * d * m
